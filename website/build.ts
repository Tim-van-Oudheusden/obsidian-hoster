...
import cfg from "../website.config"
...
async function buildWebsite(argv: Argv, mut: Mutex, clientRefresh: () => void) {
  const ctx: BuildCtx = {
...
export default async (argv: Argv, mut: Mutex, clientRefresh: () => void) => {
  try {
    return await buildWebsite(argv, mut, clientRefresh)
  } catch (err) {
    trace("\nExiting Website due to a fatal error", err as Error)
  }
}
